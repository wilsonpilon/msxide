...
`Compilar (IDE) <DIGNIFIED_CODE> [CLASSIC_CODE] [arguments]`
...
> `Compilar (IDE) d:\msxapps\testdisk\test.dmx -ss -ca -lbr --tk_tokenize --em_run --em_monitor`  
...
- **Labels**  
  ...
  Flow visualization with `Compilar (IDE) labels.dmx -lbr`  
   `BASIC  
    ...
    80 END  
	`  
   Report generated with `Compilar (IDE) labels.dmx -lnr`  
   `  
	8 lines generated.  
	(Classic - Dignified)  
	10 - 2  
...
- **Defines**  
  ...
  **CoCo:** `PRINT@c,`  
  Depending on the content of `(x,y)`, `c` can be:  
   ...
   - - `y` and not `x`: `x` = `0`  
   - - `x` and `y` are only one number each: the result of _32 _ y + x*  
  ...
  **One** and **two** letters variables used directly will **not** be **converted**. Warnings and/or errors will be given in case of conflicts.  
  ...
  A **summary** of the long and short name **associations** can be generated by using the `-var` argument. It will be saved to a file but can be logged to the console using the `-prr` (print report) argument.  
   >The conversion (and the **syntax highlight**) will catch illegal variables when declaring. Repeated declarations will cause an error. A warning will be given if **hardcoded** short named variables are **already** assigned and if a short name hard coded was already used by **substitution**. Warnings will also nag you if the first two letters between reserved long named and/or short named variables clashes.
      ...
      40 PRINT ZZ$
      ```
      Report generated with  `Compilar (IDE) vars.dmx -var`
      ```
...
- _Destination file_  
  ...
  If no destination is given, the _source_ name will be used with the appropriate extension.  
  If only a file name is given, the file will be saved on the _source_ folder.  
  if only a path name is given, the file will be saved on that path with the _source_ name.  
...
- _Add a header about Basic Dignified_  
...
- _Translate special ASCII characters_  
...
- _Convert `PRINT` or `?`_ **(this is a feature of the MSX and CoCo modules)**  
...
- _Strip adjacent `THEN`/`ELSE` or `GOTO`s_ **(this is a feature of the MSX and CoCo modules)**  
...
- #### **The command line arguments**
  ...
  ```
  usage: Compilar (IDE) [input] [output]
...
- #### **The remtags**
  ...
  - `export_file` is a new file and path to **replace** the current destination file and path. Useful to easily test different **versions** of the code without overriding the previous one or setting a different save path. All the rules of the _destination_ argument are applied here as well.
